...
export class WebSocketServer {
  ...
  private sendToWebsocket(ws: WebSocket, data: Record<string, any>): void {
  ...
  private async handleMessage(ws: WebSocket, row: Record<string, any>): Promise<void> {
    ...
    if (action === 'open' && recordType === 'library' && !exists) {
      ...
      try {
        const dbService = await this.loadLibrary(library);
    ...
    const handler = await WebSocketRouter.route(dbService, ws, {
      ...row,
      ...payload
    });
